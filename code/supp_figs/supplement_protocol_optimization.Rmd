...
df_po <- read_csv("../../data/2022_09_15_PO_bonds_orbit.csv") %>% mutate(eff = Kan_count / LB_count)
...
```{r}
df_pur <- read_csv("../../data/2022_09_07_TO_purification.csv") %>% mutate(eff = Kan_count / LB_count) %>% 
...
```{r}
df_co <- read_csv("../../data/2022_10_10_oligo_supplier.csv") %>% mutate(eff = Kan_count / LB_count)  %>% 
...
save_plot("../../figures/supplement/supp_protocol_optimization.pdf", plot = supp_protocol, base_height = 3, base_width = 7)
```
